...
class TestConstruction:
    def test_main_only(self, results_pair):
        pt = paper_tables(main=[results_pair["r1"], results_pair["r2"]], template="aer")
...
class TestSerialisation:
    ...
    def test_writes_markdown_via_filename(self, results_pair):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            paper_tables(main=[results_pair["r1"]], template="aer", filename=path)
            assert os.path.exists(path)
            with open(path, encoding="utf-8") as f:
...
def test_paper_tables_repr(results_pair):
    pt = paper_tables(
        main=[results_pair["r1"]], placebo=[results_pair["r_placebo"]], template="qje"
    )
    r = repr(pt)
