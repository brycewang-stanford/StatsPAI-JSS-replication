...
*
* Module 56: Three-way cluster-robust OLS SE.
*   StatsPAI:  sp.multiway_cluster_vcov
*   R:         sandwich::vcovCL(cluster = ~ g1 + g2 + g3, type="HC1", cadjust=TRUE)
...
do _common.do
stata_parity_init, module(56_multiway_cluster)
stata_parity_open, module(56_multiway_cluster)

import delimited "${STATA_PARITY_DATA}/56_multiway_cluster.csv", clear case(preserve)

reghdfe y x, noabsorb vce(cluster g1 g2 g3)
...
mata:
real matrix sp_mw56_oneway_vcov(real matrix X, real colvector u, real matrix C)
{
...
real matrix sp_mw56_vcov()
{
    real matrix X, C1, C2, C3, V
    ...
    C3 = st_data(., "g3")
    V =
        sp_mw56_oneway_vcov(X, u, C1) +
        sp_mw56_oneway_vcov(X, u, C2) +
        sp_mw56_oneway_vcov(X, u, C3) -
        sp_mw56_oneway_vcov(X, u, (C1, C2)) -
        sp_mw56_oneway_vcov(X, u, (C1, C3)) -
        sp_mw56_oneway_vcov(X, u, (C2, C3)) +
        sp_mw56_oneway_vcov(X, u, (C1, C2, C3))
    return(0.5 * (V + V'))
}

st_matrix("VBRIDGE", sp_mw56_vcov())
end
...
stata_parity_extra, key(method) val("audited Stata/Mata three-way cluster bridge")
stata_parity_extra, key(stata_command) val("reghdfe y x, noabsorb vce(cluster g1 g2 g3) for diagnostic SEs; headline SEs from Mata CGM bridge")
stata_parity_extra, key(stata_bridge_status) val("audited Stata/Mata algorithm bridge")
stata_parity_extra, key(stata_algorithm) val("Cameron-Gelbach-Miller inclusion-exclusion over all nonempty cluster intersections with per-component Liang-Zeger CR1 correction matching sandwich::vcovCL HC1/cadjust")
...
stata_parity_close, module(56_multiway_cluster)
