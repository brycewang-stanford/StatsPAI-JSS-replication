...
def plot_residuals(
    ...
    ax = axes[1, 1]
    if (
        result._panel_data is not None
        and result._time is not None
        and result._lm_result is not None
    ):
        try:
            resid_series = result._lm_result.resids
...
def plot_compare(
    ...
    for i, (name, r) in enumerate(results.items()):
        ...
        ci_hi: List[float] = []
        for var in variables:
            c = float(r.params.get(var, np.nan))
            s = float(r.std_errors.get(var, np.nan))
            coefs.append(c)
...
def plot_hausman(
    ...
    if (
        result._panel_data is None
        or result._formula is None
