...
def _detect_method(result: Any) -> str:
    ...
    # Classical methods
    if any(
        k in method_str for k in ("did", "diff", "callaway", "sun_abraham", "stagger")
    ):
        return "did"
    ...
    if any(
        k in method_str
        for k in ("panel", "fe ", "re ", "fixed effect", "random effect")
    ):
        return "panel"
        ...
        output["checks"].append(
            {
                ...
                "interpretation": (
                    "Balanced panel"
                    if n_entities and n_time
                    else "Panel info available"
                ),
            }
        )
...
def _battery_iv(result: Any, alpha: float = 0.05) -> Dict[str, Any]:
    ...
    # First-stage F
    fs_f = _get_diagnostic(result, "First-stage F") or _get_model_info(
    ...
    sargan = _get_diagnostic(result, "Sargan statistic") or _get_model_info(
        result, "sargan_stat"
    )
    sargan_p = _get_diagnostic(result, "Sargan p-value") or _get_model_info(
        result, "sargan_pvalue"
    )
    ...
    # Wu-Hausman
    hausman = _get_diagnostic(result, "Wu-Hausman") or _get_model_info(
        result, "wu_hausman"
    )
    hausman_p = _get_diagnostic(result, "Wu-Hausman p-value") or _get_model_info(
        result, "wu_hausman_pvalue"
    )
        ...
        checks.append(
            {
                ...
                "interpretation": (
                    "Endogeneity confirmed — IV needed"
                    if float(hausman_p) < alpha
        ...
        checks.append(
            {
                ...
                "interpretation": (
                    ...
                    if p_val and float(p_val) >= alpha
        ...
        checks.append(
            {
                ...
                + (
                    " — sufficient"
                    if int(n_left) >= 20 and int(n_right) >= 20
        ...
        checks.append(
            {
                ...
                "interpretation": (
                    "PCEs: "
                    + "; ".join(
                        f"{r['stratum']}={r['estimate']:.3f}" for _, r in eff.iterrows()
                    )
                ),
            }
        )
        ...
        checks.append(
            {
                ...
                "interpretation": (
                    f"SACE ∈ [{lo:.3f}, {hi:.3f}]"
                    if valid
                    else "SACE bounds degenerate (no always-survivors)."
                ),
            }
        )
        ...
        checks.append(
            {
                ...
                "interpretation": (
                    ...
                    + (
                        " — clean"
                        if frac < 0.1
                        else " — high failure rate; SE may be unreliable."
                    )
                ),
            }
        )
...
def _battery_mediation_interventional(
